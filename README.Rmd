...
section.1 <- "T-tests, planned contrasts, regressions, moderations, simple slopes"
section.2 <- "Nice APA tables"
...
The tables can be saved with the `save_as_docx` function, and are `flextable` objects, and can be modified as such.

```{r t_table, warning = FALSE, message = FALSE}
...
```{r contrasts_table, warning = FALSE, message = FALSE}
...
```{r mod_table, warning = FALSE, message = FALSE}
...
```{r slopes_table, warning = FALSE, message = FALSE}
...
It also integrates with objects from the `broom` and `report` packages. Full tutorial: https://remi-theriault.com/blog_table.html
