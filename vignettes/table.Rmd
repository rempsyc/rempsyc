...
So far so good; we've managed to tranform the data in a suitable format for the next step, but it's not over yet. We need to use the `flextable::separate_header` function, which will produce multilevel headings like we want, based on the periods, and depending on the position of the terms in the column names. We also have to add back some formatting to get a perfect result.
