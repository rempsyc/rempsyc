...
nice_table(mtcars[1:3,], 
           ...
           footnote = "Note: The data was extracted from the 1974 Motor Trend US magazine.")
```
...
```{r}
nice_table(stats.table, width = .50)
```
...
```{r}
my_table <- nice_table(stats.table, width = .60)
```
...
```{r}
save_as_docx(my_table, path = "D:/R treasures/nice_tablehere.docx")
```
...
```{r}
nice_table(stats.table, highlight = TRUE, width = .50)
```
...
```{r}
nice_table(stats.table, highlight = .01, width = .50)
```
...
nice_table(stats.table, broom = "lm", width = .60)
```
...
nice_table(stats.table, width = 1)
```

The `report` package provides quite comprehensive tables, so one may request an abbreviated table with the `short` argument.

```{r}
nice_table(stats.table, short = TRUE, width = .60)
```

### `rempsyc` table

`nice_table` also integrates nicely with other functions from the `rempsyc` package: [`nice_t_test`](https://remi-theriault.com/blog_t-test), [`nice_mod`, `nice_slopes`, `nice_lm`, and `nice_lm_slopes`](https://remi-theriault.com/blog_moderation), because they provide good default formats that include effect sizes. Let's make a quick demo for some of them. The t-test function supports making several t-tests at once by specifying the desired dependent variables.

#### t-tests: [`nice_t_test`](https://remi-theriault.com/blog_t-test)
...
nice_table(stats.table, width = .60)
```

#### Moderations: [`nice_mod`](https://remi-theriault.com/blog_moderation)
...
```{r}
nice_table(test[8:11], col.format.p = 1:4, width = .25)
```
...
nice_table(test[8:11], col.format.custom = 2:4, format.custom = "fun", width = .20)
```
...
## Special situation: multiple headers
...
So assuming we have a study with several time measurements, we will make a copy of the iris data set and pretend this is the "Time 2". Species will be our grouping variable.
...
names(wide.data)[-1] <- paste0(rep(c("T1.", "T2."), each=6),
                               rep(descriptive.data$Species, 
                                   times=2, each=2),
                               paste0(c(".M",".SD")))

# Make preliminary nice_table
my.table <- nice_table(wide.data)
my.table

```

So far so good; we've managed to tranform the data in a suitable format for the next step, but it's not over yet. We need an extra package to do this, `ftExtra`. We're only really interested in the `span_header` function, but because it doesn't use the same theme/style as we do, we have to add back some of it manually.
...
my.table %>%
  ...
  italic(i = 3, part = "header") -> my.table
my.table

```

If you find a more efficient way to do this (especially the data wrangling part), please let me know. Nice result though!
