...
Identify outliers based on 3 median absolute deviations (MAD) from the
median.
...
- mad.scores:
  ...
  Logical, whether to output robust z (MAD) scores (default) or raw
  scores. Defaults to `TRUE`.

## Value

A list of dataframes of outliers per variable, with row numbers, based
on the MAD. When printed, provides the number of outliers, selected
variables, and any outlier flagged for more than one variable. More
information can be obtainned by using the
[`attributes()`](https://rdrr.io/r/base/attributes.html) function around
...
to "standardize" the data based on the MAD and median, and then check
for any observation greater than the specified criteria (e.g., +/-3).
