...
#'
#' @description Formats output of `lm` model object for a publication-ready format.
#'
...
nice_lm <- function(model,
  ...
  response.names <- unlist(lapply(models.list, function(x) {rep(as.character(x$terms[[2]]),
                                                         each=length(x$assign)-1)}))
