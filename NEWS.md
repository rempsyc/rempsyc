...
## rempsyc 0.1.9.4
* `nice_lm_slopes()`: add explicit test documenting that continuous moderators work correctly even when models contain factor covariates with multiple levels
...
* `nice_lm_slopes()`: add better error message when factor moderators are used, explaining that only continuous moderators are supported
