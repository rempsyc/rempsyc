...
## rempsyc 0.1.0.3
* `nice_table` now only keeps the first occurence of repeated (duplicated) dependent variables, and merges (and centers). Only works for a column called "Dependent Variables". For columns named "Predictor", now also converts colons ":" to the multiplication symbol, "×".
