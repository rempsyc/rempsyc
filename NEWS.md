...
## rempsyc 0.1.9.7
* `nice_table()`: add `use.labels` parameter to use variable labels as column names when available. Variable labels are expected to be stored as "label" attributes on data frame columns. This addresses the long-standing TODO item for variable label support in publication-ready tables.
